...
const target = z.object({ enabled: z.boolean().default(true) }).default({ enabled: true });
export const WorkflowSchema = z.object({
  ...
  targets: z.object({ claude: target, codex: target, agents: target, cursor: target, copilot: target }).partial().default({}),
}).passthrough();
...
export function validateSchema(data: unknown): { workflow?: RawWorkflow; diagnostics: Diagnostic[] } {
  ...
  if (data && typeof data === 'object' && !Array.isArray(data)) for (const k of Object.keys(data)) if (!known.has(k)) diagnostics.push({ severity:'warning', path:k, message:`Unknown top-level field '${k}' will be ignored by renderers.` });
